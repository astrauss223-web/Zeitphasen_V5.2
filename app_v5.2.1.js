...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function exportProposalPdf() {
        ...
        const fileName = `Anlagevorschlag_V5.2.1_${dateStr.replace(/\./g, '-')}.pdf`;

        doc.setFontSize(18); doc.setTextColor(3, 61, 93);
        doc.text('Anlagevorschlag - Zeitphasenmodell V5.2.1', 14, 20);
...
function getSavedLibrary() {
    try {
        return JSON.parse(localStorage.getItem('vem_library_v5.2.1') || localStorage.getItem('vem_library_v5.2') || '[]');
    } catch(e) {
        return [];
    }
}
...
function saveSavedLibrary(items) {
    try {
        localStorage.setItem('vem_library_v5.2.1', JSON.stringify(items));
    ...
    libraryList.querySelectorAll('.btn-lib-export').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (items[idx]) {
                ...
                dlAnchor.setAttribute("download", `${(item.name || 'depot').replace(/\s+/g, '_')}_v5.2.1.json`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            ...
            dl.setAttribute("download", `vem_setup_v5.2.1_${new Date().toISOString().slice(0,10)}.json`);
